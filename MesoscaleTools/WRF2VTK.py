...
def getSRatio(f):
    try:
        ...
        print("WARNING: could not determine subgrid ratio, using 1.", file=sys.stderr)
        srx=1
        sry=1
    return srx,sry
...
def readGrid(file,gridname,itime=0):
    ...
    try:
        ...
        print("Could not read spatial resolution, setting to 1.", file=sys.stderr)
        dx=1
        dy=1
    if gridname == 'fire':
...
def main(files,variables,vectors,time=-1,nocompress=False,output='wrfout'):
    ...
    for f in files:
        ...
        if time == -1:
            ...
            time=[time]
        allvars=sum(list(vectors.values()),[])
